...
public class InputReaderEncodedTest {
    ...
    public void readFromStringXml() throws InvalidInputException {
        ...
        Values values = inputReader.readString("<?xml version=\"1.0\" encoding=\"UTF-8\"?> \n" +
                "<values>  \n" +
                "  <value>  \n" +
                "    <encodedData format=\"SML_EDL40_P\" encoding=\"base64\">TEST1</encodedData>  \n" +
                "    <publicKey encoding=\"base64\">TEST2</publicKey>  \n" +
                "  </value> \n" +
                "</values");
        Assert.assertEquals(1, values.getValues().size());
        Value value = values.getValues().get(0);
        Assert.assertEquals("SML_EDL40_P", value.getEncodedData().getFormat());
        Assert.assertEquals("base64", value.getEncodedData().getEncoding());
        Assert.assertEquals("TEST1", value.getEncodedData().getValue());
    ...
    public void readFromStringXmlTwoValues() throws InvalidInputException {
        ...
        Values values = inputReader.readString("<?xml version=\"1.0\" encoding=\"UTF-8\"?> \n" +
                "<values>  \n" +
                "  <value>  \n" +
                "    <encodedData format=\"SML_EDL40_P\" encoding=\"base64\">TEST1</signedData>  \n" +
                "    <publicKey encoding=\"plain\">TEST2</publicKey>  \n" +
                "  </value> \n" +
                "  <value>  \n" +
                "    <encodedData format=\"SML_EDL40_P\" encoding=\"base64\">TEST3</signedData>  \n" +
                "    <publicKey encoding=\"base64\">TEST4</publicKey>  \n" +
                "  </value> \n" +
                "</values");
        Assert.assertEquals(2, values.getValues().size());
        Value value1 = values.getValues().get(0);
        Assert.assertEquals("SML_EDL40_P", value1.getEncodedData().getFormat());
        Assert.assertEquals("base64", value1.getEncodedData().getEncoding());
        Assert.assertEquals(EncodingType.BASE64, value1.getEncodedData().getEncodingType());
        Assert.assertEquals("TEST1", value1.getEncodedData().getValue());
        ...
        Assert.assertEquals("SML_EDL40_P", value2.getEncodedData().getFormat());
        Assert.assertEquals("base64", value2.getEncodedData().getEncoding());
        Assert.assertEquals("TEST3", value2.getEncodedData().getValue());
        Assert.assertEquals("TEST4", value2.getPublicKey().getValue());
        Assert.assertEquals("base64", value2.getPublicKey().getEncoding());
        Assert.assertEquals(VerificationType.EDL_40_P, value2.getEncodedData().getFormatAsVerificationType());
        Assert.assertEquals(EncodingType.BASE64, value2.getEncodedData().getEncodingType());
    }
